...
static oreclazy_mg o_oreclazy_mg;
...
protected:
  ...
  static bool validate(gtm_thread *tx)
  {
    ...
    // most recent orec updates before the global time that we load.
        ...
        // number bits.
        if (oreclazy_mg::get_time(o) != oreclazy_mg::get_time(i->value)
            && o != locked_by_tx)
          return false;
      }
    return true;
  ...
  static gtm_word extend(gtm_thread *tx)
  {
    ...
    gtm_word snapshot = o_oreclazy_mg.time.load(memory_order_acquire);
    if (!validate(tx))
    ...
    do
      {
        ...
        gtm_word o = o_oreclazy_mg.orecs[orec].load(memory_order_acquire);

        if (likely (!oreclazy_mg::is_more_recent_or_locked(o, snapshot)))
          {
            success:
        ...
        else if (!oreclazy_mg::is_locked(o))
          {
...
public:
  ...
  virtual gtm_restart_reason begin_or_restart()
  {
    ...
    if (snapshot >= o_oreclazy_mg.TIME_MAX)
  ...
  virtual bool trycommit(gtm_word& priv_time)
  {
    ...
    gtm_word ct = o_oreclazy_mg.time.fetch_add(1, memory_order_acq_rel) + 1;
    ...
    gtm_word v = oreclazy_mg::set_time(ct);
  ...
  virtual bool supports(unsigned number_of_threads)
  {
    ...
    return (number_of_threads * 2 <= oreclazy_mg::OVERFLOW_RESERVE);
  }

  CREATE_DISPATCH_METHODS(virtual, )
  CREATE_DISPATCH_METHODS_MEM()

  oreclazy_wt_dispatch() : abi_dispatch(false, true, false, false, 0, &o_oreclazy_mg)
  { }
};

} // anon namespace

static const oreclazy_wt_dispatch o_oreclazy_wt_dispatch;

abi_dispatch *
GTM::dispatch_oreclazy ()
{
  return const_cast<oreclazy_wt_dispatch *>(&o_oreclazy_wt_dispatch);
}
